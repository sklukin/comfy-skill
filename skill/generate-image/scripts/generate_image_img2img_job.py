...
BOUNDARY = "----OpenClawImg2ImgBoundary7MA4YWxkTrZu0gW"
...
def main():
    p = argparse.ArgumentParser(description="Submit and wait for an Images API img2img job")
    p.add_argument("--base-url", default=DEFAULT_BASE_URL)
    p.add_argument("--input", required=True, help="Path to source image")
    p.add_argument("--prompt", required=True)
    p.add_argument("--model", default="flux-dev")
    p.add_argument("--denoise", type=float, default=0.65)
    ...
    if args.model != "flux-dev":
        print("img2img is only supported with flux-dev", file=sys.stderr)
        return 2
    ...
    print(f"Timed out waiting for img2img job {job_id}", file=sys.stderr)
    return 1
