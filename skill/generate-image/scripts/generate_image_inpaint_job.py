...
def main():
    p = argparse.ArgumentParser(description="Submit and wait for an Images API inpainting job (FLUX Fill)")
    p.add_argument("--base-url", default=DEFAULT_BASE_URL)
    p.add_argument("--input", required=True, help="Path to source image")
    p.add_argument("--mask", required=True, help="Path to mask image (white = inpaint area, black = keep)")
