...
BOUNDARY = "----OpenClawVideoBoundary7MA4YWxkTrZu0gW"
...
def main():
    p = argparse.ArgumentParser(description="Submit and wait for a Wan 2.2 image-to-video job")
    p.add_argument("--base-url", default=DEFAULT_BASE_URL)
    p.add_argument("--input", required=True, help="Path to source image")
    p.add_argument("--prompt", required=True, help="Describe the desired motion/animation")
    p.add_argument("--negative-prompt", default=WAN_DEFAULT_NEGATIVE, help="Negative prompt (has good default)")
    p.add_argument("--width", type=int, default=768)
    p.add_argument("--height", type=int, default=768)
    p.add_argument("--steps", type=int, default=20)
    p.add_argument("--guidance-scale", type=float, default=3.5)
    p.add_argument("--seed", type=int, default=-1)
    p.add_argument("--output", required=True, help="Output path (.webp)")
    ...
    print(f"Video generation started (job {job_id[:8]}). This takes ~10 minutes...", flush=True)
    ...
    while time.time() < deadline:
        ...
        if current == "processing":
            elapsed = time.time() - (deadline - args.timeout_seconds)
            print(f"Processing... ({int(elapsed)}s elapsed)", flush=True)

        time.sleep(args.poll_seconds)

    print(f"Timed out waiting for video job {job_id}", file=sys.stderr)
    return 1
